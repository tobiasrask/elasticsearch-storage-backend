  ...
  _createClass(ElasticsearchStorageBackend, [{
    ...
    */
    ...
    value: function updateIndices() {
      ...
      var self = this;
      _entityApi2.default.log("ESStorageBackend", 'Udate indices');

      if (!indices.length) return Promise.resolve();

      return indices.reduce(function (sequence, indexData) {
        return sequence.then(function () {
          _entityApi2.default.log("ESStorageBackend", 'Updating index: ' + indexData.title + ' ({indexData.indexName})');

          // Allow alter for index data
          return self.prepareIndiceForUpdate(indexData).then(function () {
            return _this4.getStorageHandler().prepareIndiceForUpdate(indexData);
          }).then(function () {
            _entityApi2.default.log("ESStorageBackend", 'Closing index: ' + indexData.title + ' ({indexData.indexName})');
            return self.getElasticsearchInstance().indices.close({
              index: indexData.indexName
            });
          }).then(function () {
            _entityApi2.default.log("ESStorageBackend", 'Perform update for index: ' + indexData.title + ' ({indexData.indexName})');
            return self.getElasticsearchInstance().indices.putSettings({
              index: indexData.indexName,
              body: indexData.settings
            });
          }).then(function () {
            _entityApi2.default.log("ESStorageBackend", 'Opening index: ' + indexData.title + ' ({indexData.indexName})');
            return self.getElasticsearchInstance().indices.open({
              index: indexData.indexName
            });
          }).then(function (result) {
            _entityApi2.default.log("ESStorageBackend", 'Index updated: ' + indexData.title + ' (' + indexData.indexName + ')');
            return Promise.resolve();
          }).catch(function (err) {
            _entityApi2.default.log("ESStorageBackend", err.toString(), 'error');
            console.error(err);
    ...
    */
    ...
    key: 'prepareIndiceForUpdate',
    value: function prepareIndiceForUpdate(indexData) {
      return Promise.resolve();
    }
    ...
    */
    ...
    value: function updateMappings() {
      ...
      var self = this;
      _entityApi2.default.log("ESStorageBackend", 'Update mappings');

      if (!mappings.length) return Promise.resolve();

      return mappings.reduce(function (sequence, mappingData) {
        return sequence.then(function () {
          _entityApi2.default.log("ESStorageBackend", 'Updating mapping: ' + mappingData.title + ' ({mappingData.typeName})');
          ...
          return self.getElasticsearchInstance().indices.putMapping({
            ...
            _entityApi2.default.log("ESStorageBackend", 'Mapping updated: ' + mappingData.title + ' (' + mappingData.typeName + ')');
            return Promise.resolve();
          }).catch(function (err) {
            _entityApi2.default.log("ESStorageBackend", err.toString(), 'error');
    ...
    */
    ...
    value: function updateSchemas(schemas, options, callback) {
      ...
      var self = this;
      var errors = false;
      this.updateIndices(schemas.indices).then(function () {
        return _this5.updateMappings(schemas.mappings);
      }).then(function () {
        callback(null);
      }).catch(function (err) {
        callback(err);
